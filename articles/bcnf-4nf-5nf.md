...
BCNF では「全ての決定項が候補キー」であることを要求します。ここで言う決定項（determinant）とは、関数従属性 `X -> Y` において左側の属性集合 `X` を指します。つまり **「どの属性組み合わせも、他の列を一意に決められるなら必ず候補キーでなければならない」** という強い制約です。候補キー以外が決定項になると、更新・削除・挿入の各種異常が温存されるため、BCNF ではそれを許しません。

`event_assignments` を以下の 2 テーブルに分割し、イベントと会場担当の従属性を切り離します。
...
`artist_skills` では、アーティストごとに複数のスキルと複数の担当楽器が独立に存在します。すなわち `artist_id ->> skill` と `artist_id ->> instrument` という多値従属性が同居しており、**1レコードで 2 種類の繰り返し属性を保持している状態**です。このように、1つのキーから独立した複数の繰り返し属性が導かれる関係を多値従属性といいます。

### 2-2. 4NF への分解

多値従属性ごとにテーブルを分け、交差表を排除します。ここでいう交差表とは、今回の `artist_skills` のように「1つのキー列に対して、独立した2種類以上の属性を組み合わせた行を持ち、実質的に直積を保持しているテーブル」を指します。スキルと楽器を別テーブルに分けることで、この直積構造をなくし、4NF の要件を満たせます。
